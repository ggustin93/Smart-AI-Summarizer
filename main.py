...
def extract_text(url, file_index):
    try:
        ...
        os.makedirs("outputs/extracted_texts", exist_ok=True)
        with open(f"outputs/extracted_texts/extracted_text_{file_index}.md",
                  "w",
                  encoding="utf-8") as file:
            file.write(text)
        logging.debug(
            f"Texte extrait et sauvegardé dans outputs/extracted_texts/extracted_text_{file_index}.md"
        )
        return text
...
def call_corcel_api(messages, retries=3, delay=10):
    ...
    for attempt in range(retries):
        try:
            ...
            logging.debug(
                f"Réponse de l'API Corcel : {json.dumps(response_data, indent=2)}"
            )
...
def optimize_query_with_corcel(user_query):
    messages = [{
        ...
        "system",
        "content":
        "Vous êtes un expert en recherche Google. Utilisez des opérateurs de recherche avancés pour optimiser la requête suivante. L'output doit être tel qu'entré directement dans le moteur de recherche."
    }, {
        "role":
        "user",
        "content":
    ...
    # Extraire la requête optimisée en utilisant des expressions régulières
    match = re.search(r'\"(.*?)\"', optimized_query)
    optimized_query = match.group(
        1).strip() if match else optimized_query.strip()

    # Nettoyer la requête pour s'assurer qu'elle soit concise et directement applicable
...
def summarize_page_with_corcel(text, expertise):
    messages = [{
        ...
        "user",
        "content":
        f"Veuillez faire une synthèse concise en bullet points des informations suivantes : {text}"
    }]
    return call_corcel_api(messages)
...
def guess_expertise_with_corcel(query):
    messages = [{
        ...
        "user",
        "content":
        f"Veuillez deviner l'expertise la plus pertinente pour la requête suivante : {query}"
    }]
...
def create_mega_synthesis_with_corcel(summaries, expertise):
    combined_summaries = "\n\n".join(summaries)
    messages = [{
        "role":
        "system",
        ...
        "user",
        "content":
        f"Veuillez rédiger une méga-synthèse basée sur les synthèses suivantes : {combined_summaries}"
    }]
    return call_corcel_api(messages)
...
def main():
    ...
    for index, url in enumerate(search_results):
        ...
        with open(f"outputs/syntheses/synthese{index + 1}.md",
                  "w",
                  encoding="utf-8") as file:
            file.write(summary)
        logging.debug(
            f"Synthèse sauvegardée dans outputs/syntheses/synthese{index + 1}.md"
        )

    # Création de la méga-synthèse via l'API Corcel
    mega_synthesis = create_mega_synthesis_with_corcel(summaries, expertise)
    mega_synthesis += "\n\nSources:\n" + "\n".join(sources)
    os.makedirs("outputs", exist_ok=True)
    with open("outputs/mega_synthese.md", "w", encoding="utf-8") as file:
        file.write(mega_synthesis)
    logging.debug("Méga-synthèse sauvegardée dans outputs/mega_synthese.md")
